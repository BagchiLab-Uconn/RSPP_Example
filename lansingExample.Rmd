...
mod_sm <- klmer(formula = pppx ~ 1  + marks + (1|subPlotID),
                     r=r, hyper=hf,  correction='border', weights_type = "nx_A",
                     na.action="na.omit")
...
mod_sm_cis <- confint(mod_sm, level=0.95, newdata = preddat, nboot=100, ncore=4, iseed=1234)

plot(mod_sm_cis)

mod_sm_preds <- as.data.frame.table(mod_sm_cis$predictions)

mod_sm_preds <- pivot_wider(mod_sm_preds, names_from = Var3, values_from = Freq) %>%
  mutate(marks = preddat$marks[as.numeric(Var2)],
         distance = as.numeric(as.character(Var1)))
head(mod_sm_preds)

ggplot(mod_sm_preds, aes(x = distance, y = est, ymin = lwr, ymax = upr)) + 
         facet_wrap(~marks) + 
         geom_ribbon(alpha = 0.1) +geom_path() +
